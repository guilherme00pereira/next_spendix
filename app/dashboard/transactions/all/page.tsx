...
const AllTransactions = async ({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) => {
  ...
  queryTransactions = searchParams.income ? transactions.filter((t) => t.categories?.type === "Receita") : transactions;

  queryTransactions = searchParams.spendings ? transactions.filter((t) => t.categories?.type === "Despesa") : queryTransactions;

  queryTransactions = searchParams.day ? transactions.filter((t) => t.due_date.substr(-2) === searchParams.day) : queryTransactions;

  return (
    <PageContainer
      title="Todas as transações do mês"
      showSelectMonthYear
      breadcrumb={<Breadcrumbs steps={[{ title: "Transações", href: "dashboard/transactions" }, { title: "Todas as transações" }]} />}
    >
        ...
        <AllTransactionsPageProvider>
          ...
          <Stack direction={{ xs: "column", md: "row" }} sx={{ width: "100%" }} justifyContent="center">
            <Suspense fallback={<p>loading</p>}>
              <TransactionsTable title="Transações" filters={<TransactionsFilter transactions={queryTransactions} />}>
